# Hey, I'm Alpha 👋  

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,100:2c5364&height=200&section=header&text=Alpha%20%7C%20Cyber%20%2B%20Web3&fontSize=35&fontColor=ffffff&animation=fadeIn" />
</p>

<!-- Typing -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=00FFB3&center=true&vCenter=true&width=600&lines=Break+%7C+Secure+%7C+Build;Cloud+Security;Web3+%2F+Crypto;AI+Automation" />
</p>

Cybersecurity Analyst | Cloud Security | Web3 Builder  

I build and break systems — focusing on security, automation, and scalable architectures.

---

## ⚡ What I Do

• 🔐 Cybersecurity (VAPT, API Security, Pentesting)  
• ☁️ Cloud Security (AWS, IAM, Infrastructure Security)  
• 🔗 Blockchain & Crypto Systems  
• 🤖 AI + Security Automation  

---

## 🚀 Current Focus

🔭 Building: AI-powered compliance & security systems  
🛠 Working on: Secure fintech + payment architectures  
📚 Learning: Advanced cloud security + OSCP prep  

---

## 🧠 Tech Stack

**Security:**  
VAPT • Burp Suite • Nmap • OWASP Top 10 • API Security  

**Cloud:**  
AWS • IAM • VPC • CloudTrail • WAF  

**Dev & Automation:**  
Python • Docker • Terraform • CI/CD  

**Blockchain:**  
Web3 Basics • Crypto Systems • Smart Contract Security (learning)  

**AI:**  
AI Agents • Automation • LLM Integrations  

---

## 🧩 Featured Projects

### 🔐 REGWatch  
Multi-agent compliance detection system where AI agents analyze risks and prevent regulatory issues  

→ Focus: AI + Security + Automation  

---

### 🐳 DockScanner  
Docker image vulnerability & malware scanner  

→ Focus: Container Security  

---

### ⚔️ Python Brute Force  
Custom brute-force testing tool for authentication systems  

→ Focus: Offensive Security  

---

### 🌐 CryptoSimplified  
Educational crypto + blockchain resource platform  

→ Focus: Web3 + Knowledge Systems  

---

### 📝 Personal Blog  
Writeups on security, systems, and development  

→ Focus: Knowledge Sharing  

---

## 🧪 Research & Interests

• Cloud misconfiguration exploitation  
• API authentication bypass techniques  
• Secure fintech system design  
• AI in cybersecurity automation  

---

## 🌐 Connect

📫 Email: your-email  
🔗 LinkedIn: your-link  
🌍 Portfolio: https://name-jashuva.netlify.app  

---

## ⚠️ Mindset

Security is not a feature. It's the foundation.  
Build secure. Break smart. Repeat.

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=jash777&theme=github_dark" />
</p>

